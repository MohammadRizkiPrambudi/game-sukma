...
if (isset($_POST["jawaban"])) {
  ...
  if ($hitung > 0) {
    mysqli_query($koneksi, "UPDATE tbl_jawaban SET jawabanpost = '$jawaban'WHERE id_user = '$id_user' AND id_soal='$id_soal'");
  }else{
    mysqli_query($koneksi, "INSERT INTO tbl_jawaban(id_jawaban,id_user,id_soal,id_kategori,jawabanpost) VALUES('','$id_user','$id_soal','$id_kategori','$jawaban')");
  }
        ...
        $.ajax({
          ...
          success: function() {
            if (<?= $_GET["halaman"] ?> == <?= $total_halaman; ?>) {
              ...
              window.location.href = 'soalpost.php?halaman=<?= $next; ?>';
            }
          }
        });
      });
